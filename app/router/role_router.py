...
class RoleRouter(BaseCRUDRouter):
    def __init__(self, prefix: str = "", tags: List[str] = []):
        ...
        self.dao: RoleDAO = RoleDAO(
            nesting_degree=BaseCRUDRouter.IMMEDIATE_CHILD, excludes=["users"]
        )
    ...
    def register_routes(self):
        ...
        async def add_permission(
            ...
            role = await self.dao.add_role_permission(
                db_session=db, role_alias=role_alias, permission_alias=permission_alias
            )

            if role is None:
                raise HTTPException(
                    status_code=404, detail="Error adding permission to role."
                )
            return role
