    ...
    if load_parent_relationships:
        ...
        for relationship in inspect(sqlalchemy_model).relationships:
            if (
                ...
                and relationship.key not in default_excludes
            ):
                sub_model = create_pydantic_model_from_sqlalchemy(
                    relationship.mapper.class_,
                    load_parent_relationships,
                    load_child_relationships,
                    visited_models,
                    excludes,
                    level,
                )
                relationship_type = (
                    List[sub_model] if relationship.uselist else sub_model
                )
...
class BaseCRUDRouter(Generic[DBModelType]):
    ...
    def add_get_all_route(self):
        ...
        async def get_all(
            ...
            previous_link = (
                f"{base_url}?limit={limit}&offset={previous_offset}"
                if offset > 0
                else None
            )
            ...
            # check if anything was passed on from dao
            meta = reduce(
                lambda acc, item: {**item[1], **acc}
                if isinstance(item, tuple) and "meta" in item and item[1] is not None
                else meta,
                items,
                {},
            )

            dynamic_model = create_pydantic_model_from_sqlalchemy(
                self.dao.model,
                load_parent_relationships=self.dao.load_parent_relationships,
                load_child_relationships=self.dao.load_child_relationships,
                excludes=self.dao.excludes,
            )

            if self.dao.load_child_relationships:
                return DAOResponse[List[Any]](
                    success=True,
                    data=[self.dao.decompose_dict(item.to_dict()) for item in items],
                    meta=meta,
                )
    ...
    def add_get_route(self):
        ...
        async def get(
            ...
            if self.dao.load_child_relationships:
                return DAOResponse[Any](
                    success=True, data=self.dao.decompose_dict(item.to_dict())
                )

            return (
                item
                if isinstance(item, DAOResponse)
                else DAOResponse[Any](
                    success=True,
                    data=dynamic_model.model_validate(
                        item, strict=False, from_attributes=True
                    ),
                )
            )
    ...
    def add_create_route(self):
        ...
        async def create(
            item: self.create_schema, db: AsyncSession = Depends(self.get_db)
        ) -> DAOResponse:
            try:
                # TODO: Changed this from item.dict() to item.model_dump()
                item = await self.dao.create(db_session=db, obj_in=item.model_dump())

                return (
                    item
                    if isinstance(item, DAOResponse)
                    else DAOResponse[Any](success=True, data=item.to_dict())
                )
