...
class AddressDAO(BaseDAO[AddressModel]):
    ...
    @override
    async def update(self, db_session: AsyncSession, db_obj: AddressModel, obj_in: Dict[str, Any]):
    ...
    async def add_entity_address(self, db_session: AsyncSession, entity_id: UUID, address_obj: Union[Address, AddressBase], entity_model=None) -> Optional[AddressModel| dict]:
        entity_address_dao = EntityAddressDAO()

        try:
            # Check if the address already exists
            existing_address = await self.query(db_session=db_session, filters={self.primary_key: address_obj.address_id}, single=True, options=[selectinload(AddressModel.users)]) if self.primary_key in address_obj.model_fields else None
            ...
            if existing_address:
                ...
                address : AddressModel = await self.update(db_session=db_session, db_obj=existing_address, obj_in=addr_data.model_dump())
                ...
                address : AddressModel = await self.create(db_session=db_session, address_data=address_obj)
