...
class MaintenanceRequestDAO(BaseDAO[MaintenanceRequest]):
    ...
    @override
    async def create(
        self, db_session: AsyncSession, obj_in: MaintenanceRequestCreateSchema
    ) -> DAOResponse[MaintenanceRequestResponse]:
        try:
            # extract base information
            maintenance_request_info = self.extract_model_data(
                obj_in, MaintenanceRequestBase
            )
            new_maintenance_request: MaintenanceRequest = await super().create(
                db_session=db_session, obj_in={**maintenance_request_info}
            )
            ...
            return DAOResponse[MaintenanceRequestResponse](
                success=True,
                data=MaintenanceRequestResponse.from_orm_model(new_maintenance_request),
            )
    ...
    ) -> DAOResponse[List[MaintenanceRequestResponse]]:
    ...
    async def update(
        ...
        obj_in: MaintenanceRequestUpdateSchema,
    ) -> DAOResponse[MaintenanceRequestResponse]:
        entity_data = obj_in.model_dump(
            exclude_none=True, exclude=["task_number", "id"]
        ).items()
