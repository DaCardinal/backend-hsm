...
class MediaDAO(BaseDAO[MediaModel]):
    ...
    async def create(self, db_session: AsyncSession, obj_in: Union[MediaCreateSchema | Dict], media_store: str = None) -> DAOResponse:
        try:
            ...
            media_info = self.extract_model_data(obj_in, MediaBase)
            
            # upload base64 image to cloudinary and get url
            base64_data = media_info.get('content_url')
            content_url = self.upload_to_cloudinary(base64_image=base64_data, file_name=media_info.get('media_name'), media_type=media_store.lower())
            media_info['content_url'] = content_url if content_url else None
    ...
    @override
    async def update(self, db_session: AsyncSession,  db_obj: Media, obj_in: MediaUpdateSchema) -> DAOResponse[MediaResponse]:
        try:
            # specify calling class
            media_store = self.model.__name__ if media_store is None else media_store
            ...
            # extract base information
            media_info = self.extract_model_data(obj_in, MediaBase)
            # media_data = self.extract_model_data(obj_in.model_dump(exclude=['address', 'media', 'ammenities']), MediaBase)
            ...
            for media_item in media_info:
                ...
                if existing_media_item:
                        ...
                        await entity_media_dao.create(db_session=db_session, obj_in={
                            ...
                            "media_id": create_media_item.data.media_id
                        })
                else :
                    ...
                    if create_media_item is None:
                        create_media_item : MediaResponse = await self.create(db_session=db_session, obj_in=media_item.model_dump(), media_store=entity_model_name.lower())
