...
class UnderContractDAO(BaseDAO[UnderContract]):
    ...
    @override
    async def create(self, db_session: AsyncSession, obj_in: UnderContractCreate) -> DAOResponse[ContractResponse | Dict]:
        
        try:
            # extract base information
            under_contract_info = self.extract_model_data(UnderContractCreate(**obj_in).model_dump(), UnderContractCreate)
            ...
            client_id_query = await self.user_dao.query(db_session=db_session, filters={"user_id": client_id}, single=True)

            if not client_id_query:
                return DAOResponse(success=False, error="Client ID does not exist", data={})
            
            employee_id_query = await self.user_dao.query(db_session=db_session, filters={"user_id": employee_id}, single=True)

            if not employee_id_query:
                return DAOResponse(success=False, error="Employee ID does not exist", data={})

            # check if contract exists
            contract_id_query : Contract = await self.contract_dao.query(db_session=db_session, filters={"contract_number": contract_id}, single=True)

            if not contract_id_query:
                return DAOResponse(success=False, error="Contract ID does not exist", data={})

            # check if property exists
            property_unit_assoc_query = await self.property_unit_assoc_dao.query(db_session=db_session, filters={"property_unit_assoc_id": property_unit_assoc}, single=True)

            if not property_unit_assoc_query:
                return DAOResponse(success=False, error="Property ID does not exist", data={})
            
            details_methods = {
                'contract_info': (partial(self.contract_dao.add_contract_details, contract=contract_id_query), UnderContractCreate),
            }
            contract_info_obj = {'contract_info': obj_in}

            if set(details_methods.keys()).issubset(set(contract_info_obj.keys())):
                t = await self.process_entity_details(db_session, contract_id_query.contract_id, contract_info_obj, details_methods)
                print(t)

            # commit object to db session
            await self.commit_and_refresh(db_session, contract_id_query)
            
            return DAOResponse[ContractResponse](success=True, data=ContractResponse.from_orm_model(contract_id_query))
