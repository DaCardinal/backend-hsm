...
class UtilitiesDAO(BaseDAO[UtilitiesModel]):
    ...
    async def link_entity_to_utility(self, db_session: AsyncSession, entity_id: UUID, utility_id: UUID, utility_value: int, payment_type: str, entity_model=None):
        
        # check if payment type exists
        existing_payment_type : PaymentTypes = await self.payment_type_dao.query(db_session=db_session, filters={"payment_type_name": payment_type}, single=True)
        ...
        entity_object = {
            ...
            "billable_amount": utility_value
        }

        # check if entity utility linkage exists
        entity_item : EntityBillable = await self.enity_billable_dao.query(db_session=db_session, filters={
            "entity_assoc_id": entity_id,
            "entity_type": entity_model if entity_model else self.model.__name__,
            "billable_assoc_id": utility_id,
            "billable_type": "Utilities",
        ...
        if entity_item is None:
            result = await self.enity_billable_dao.create(db_session = db_session, obj_in = entity_object)
        else:
            entity_object['billable_assoc_id'] = entity_item.billable_assoc_id
            result = await self.enity_billable_dao.update(db_session = db_session, db_obj = entity_item, obj_in=EntityBillableSchema(**entity_object))
        
        return result
    
    async def add_entity_utility(self, db_session: AsyncSession, entity_id: str, utilities_info: Union[EntityBillableCreate | List[EntityBillableCreate] ], entity_model=None, entity_assoc_id=None) -> Optional[Utilities | UtilitiesBase | List[Utilities] | List[UtilitiesBase]]:
        try:
            results = []
            ...
            for utilities_item in utilities_info:
                ...
                existing_utilities_item : Utilities = await self.query(db_session=db_session, filters={f"{self.primary_key}": utilities_item.billable_id}, single=True)

                if not existing_utilities_item:
                    return DAOResponse(success=False, error="Utility does not exist", data={})
