...
class AuthDAO(BaseDAO[User]):
    ...
    async def google_callback(self, db_session: AsyncSession, request: Request):
        ...
        with google_sso:
            try:
                ...
                existing_user : User = await self.query(db_session=db_session, filters={"email": user.username}, single=True)

                if existing_user:
                    if not existing_user.is_verified:
                        return DAOResponse[str](success=True, data="User account not verified")
                    ...
                    if existing_user.login_provider is not None:
                        ...
                        redirect_url = "https://housekee-nz94.onrender.com/"
                        # return RedirectResponse(url=redirect_url)

                else:
                    # create a new user and set login provider for new user
                    await self.user_dao.create(db_session=db_session, obj_in={
                    ...
                    created_user : User = await self.query(db_session=db_session, filters={"email": user.username}, single=True)
                    created_user.update_last_login_time()

                    return signJWT(created_user)

            except Exception as e:
                return DAOResponse[str](success=True, data=f"Google Sign-In failed: {str(e)}")

    async def verify_password(self, db_session: AsyncSession, login_info: Login):
        current_user : User = await self.query(db_session=db_session, filters={"email": login_info.username}, single=True)
    ...
    async def user_exists(self, db_session: AsyncSession, email: str):
        current_user : User = await self.query(db_session=db_session, filters={"email": email}, single=True)
        
        return current_user
    
    # TODO: Remove attempt login
    async def attempt_login(self, db_session: AsyncSession, request : dict) -> DAOResponse[TokenExposed | str]:
        current_user : User = self.user_exists(db_session=db_session, email=request.email)
        ...
        if current_user.is_verified and (current_user.login_provider == "native" or current_user.login_provider is None):
            ...
            return DAOResponse[str](success=False, data="User account not verified or using a login provider")