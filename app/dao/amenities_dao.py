...
class AmenitiesDAO(BaseDAO[Amenities]):
    ...
    async def link_entity_to_ammenity(self, db_session: AsyncSession, property_unit_assoc_id: UUID, ammenity_id: UUID, entity_model=None):
        ...
        result : EntityAmenities = await self.entity_amenities_dao.query(db_session=db_session, filters={**entity_amenity_object}, single=True)

        # create entity amenity linkage if it doesn't exist
        if result is None:
            result = await self.entity_amenities_dao.create(db_session = db_session, obj_in = entity_amenity_object)
        
        return result
    
    async def add_entity_ammenity(self, db_session: AsyncSession, entity_id: str, amenities_info: Union[Amenities | AmenitiesBase | List[Amenities] | List[AmenitiesBase]], entity_model=None, entity_assoc_id=None) -> Optional[Amenities | AmenitiesBase | List[Amenities] | List[AmenitiesBase]]:
        try:
            results = []
            ...
            for amenities_item in amenities_info:
                ...
                existing_amenities_item : Amenities = await self.query(db_session=db_session, filters={**amenities_item.model_dump(exclude=['media'])}, single=True)
                
                if existing_amenities_item:
                    obj_data = self.extract_model_data(amenities_item.model_dump(exclude=["media"]), Amenities)
                    ...
                    crud_media_item : AmenitiesModel = await self.update(db_session=db_session, db_obj=existing_amenities_item, obj_in=AmenitiesUpdateSchema(**obj_data))
                else :
                    # Creating an ammenity
                    crud_media_item : Amenities = await self.create(db_session=db_session, obj_in=amenities_item.model_dump(exclude=["media"]))
                ...
                details_methods : dict = {
                    'media': (partial(self.media_dao.add_entity_media, entity_model=self.model.__name__, entity_assoc_id=entity_amenity.entity_amenities_id), MediaCreateSchema)
                }
                await self.process_entity_details(db_session, entity_amenity.entity_amenities_id, amenities_item.model_dump(), details_methods)
