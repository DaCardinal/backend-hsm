...
class Tour(Base):
    ...
    tour_type = Column(Enum(TourType), default=TourType.in_person, nullable=True, name="tour_type")
    status = Column(Enum(TourStatus), default=TourStatus.incoming, nullable=True, name="status")
    tour_date = Column(DateTime(timezone=True), default=func.now())
    property_unit_assoc_id = Column(UUID, ForeignKey("property_unit_assoc.property_unit_assoc_id", ondelete="CASCADE"))
    user_id = Column(UUID, ForeignKey("users.user_id", ondelete="CASCADE"), nullable=True)
