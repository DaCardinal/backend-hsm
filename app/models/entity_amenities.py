...
class EntityAmenities(Base):
    ...
    entity_amenities_id = Column(UUID(as_uuid=True), primary_key=True, unique=True, index=True, default=uuid.uuid4)
    entity_type = Column(String)
    amenity_id = Column(UUID(as_uuid=True), ForeignKey('amenities.amenity_id'))
    entity_assoc_id = Column(UUID(as_uuid=True), ForeignKey('property_unit_assoc.property_unit_assoc_id'))
