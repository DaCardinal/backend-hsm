...
class MediaDAO(BaseDAO[MediaModel]):
    ...
    async def create(
        ...
        try:
            ...
            media_info["content_url"] = (
                upload_response.data["content_url"] if upload_response.success else None
            )

            # determine image_type
            media_type = uploader_service.get_image_type()
            media_info["media_type"] = (
                media_type if media_type else media_info.get("media_type")
            )
        ...
        try:
            ...
            for media_item in media_info:
                ...
                if existing_media_item:
                    ...
                    media_upload: MediaResponse = await self.create(
                        db_session=db_session,
                ...
                # check for content url succes
                if media_upload.success is False:
