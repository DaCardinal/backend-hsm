...
class PropertyUnitDAO(BaseDAO[Units]):
        ...
        try:
            ...
            if set(details_methods.keys()).issubset(set(obj_in.keys())):
                await self.process_entity_details(
                    db_session, new_property_unit.property_id, obj_in, details_methods
                )
    ...
    async def update(
        ...
        try:
            ...
            details_methods = {
                "media": (
                    partial(
                        self.media_dao.add_entity_media,
                        entity_model=self.model.__name__,
                        entity_assoc_id=property_unit_assoc_key,
                    ),
                    media_schema,
                ),
                "amenities": (
            ...
            if set(details_methods.keys()).issubset(set(entity_data.keys())):
                await self.process_entity_details(
                    db_session,
                    existing_property_unit.property_unit_assoc_id,
                    entity_data,
                    details_methods,
                )
    ...
    async def get(
        ...
        return DAOResponse[PropertyUnitResponse](
            success=True, data=PropertyUnitResponse.from_orm_model(result)
        )
